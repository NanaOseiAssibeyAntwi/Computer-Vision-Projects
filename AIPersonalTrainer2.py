...
while True:
    ...
    image = detector.drawPose(image, draw=False)
    lmList = detector.getPosePostion(image, draw=False)

    # Right arm
    detector.findAngle(image,11, 13,15, draw=True)

    # Left Arm
    detector.findAngle(image,12, 14,16, draw=True)



    # Finding the angles of all 3 landmark points
    # if len(lmList) != 0:
    #     # print(lmList)
    #
    #     for i, a in enumerate(lmList):
    #         for j, b in enumerate(lmList):
    #             for k, c in enumerate(lmList):
    #                 if (i - j == 2) and (j - k == 2):
    #                     A = np.array(a, dtype=float)[1:]
    #                     B = np.array(b, dtype=float)[1:]
    #                     C = np.array(c, dtype=float)[1:]
    #                     print(A, B, C)
    #
    #                     BA = A - B
    #                     BC = C - B

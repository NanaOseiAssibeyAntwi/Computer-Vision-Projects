...
class PoseDetector:
    ...
    def findAngle(self, image, p1, p2, p3, draw = True):
        if len(self.lmList) != 0:
            ...
            angle = np.clip(m.degrees(m.atan2(y3-y2, x3-x2) - m.atan2(y1-y2, x1-x2)), -180, 180)

            print(angle)

            if draw:
                cv2.line(image, (x1, y1), (x2, y2), (255, 0, 255), 3)
                cv2.line(image, (x2, y2), (x3, y3), (255, 0, 255), 3)
                cv2.circle(image, (x1, y1), 10, (0, 0, 255), cv2.FILLED)
                cv2.circle(image, (x1, y1), 15, (0, 0, 255))
                cv2.circle(image, (x2, y2), 10, (0, 0, 255), cv2.FILLED)
                cv2.circle(image, (x2, y2), 15, (0, 0, 255))
                cv2.circle(image, (x3, y3), 15, (0, 0, 255))
                cv2.circle(image, (x3, y3), 10, (0, 0, 255), cv2.FILLED)
                cv2.putText(image, str(int(angle)), (x1,y1 -10), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 3)
